...
import 'package:image/image.dart' as img hide ImageData;
...
class Generator {
  ...
  List<List<int>> _toColumnFormat(img.Image imgSrc, int lineHeight) {
    final img.Image image = imgSrc.clone(); // make a copy
    ...
    final img.Image biggerImage = img.copyResize(image, width: widthPx, height: heightPx);
    img.fill(biggerImage, color: img.ColorRgb8(0, 0, 0));
    // Insert source image into bigger one
    img.compositeImage(biggerImage, image);
    ...
    while (left < widthPx) {
      ...
      final Uint8List bytes = slice.toUint8List();
      blobs.add(bytes);
      left += lineHeight;
    }
  ...
  List<int> _toRasterFormat(img.Image imgSrc) {
    ...
    final List<int> buffer = image.toUint8List();
